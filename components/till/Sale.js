...
import { WEATHER_API_STRING } from "../../API";
...
export default function Sale({ saleItems, setSaleItems, total, setTotal }) {
  ...
  const getWeather = async () => {
    let response = await fetch(WEATHER_API_STRING);
            ...
            saleItems.map((el) => {
              return (
                <View style={styles.itemContainer}>
                  ...
                  <TouchableOpacity
                    onPress={() => handleDelete(el.id, el.price)}
                  >
      ...
      <View style={styles.btnContainer}>
        <TouchableOpacity
          style={[
            btnDisabled
              ? styles.disabled
              : [styles.btn, { backgroundColor: "#027516" }],
          ]}
          disabled={btnDisabled}
          onPress={newSaleClicked}
        >
        ...
        <TouchableOpacity
          style={[
            btnDisabled
              ? styles.disabled
              : [styles.btn, { backgroundColor: "#024075" }],
          ]}
          disabled={btnDisabled}
          onPress={newSaleClicked}
        >
