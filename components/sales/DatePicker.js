...
export default function DatePicker({ day, setDay, monthID, setMonthID }) {
  ...
  const renderMonths = ({ item, index }) => (
    <TouchableOpacity style={styles.item} onPress={() => setMonthID(index)}>
      <Text style={index === monthID ? styles.monthSelected : styles.month}>
        {item}
      </Text>
    </TouchableOpacity>
  );
  ...
  const renderDays = ({ item, index }) => (
    <TouchableOpacity
      style={
        index + 1 === parseInt(day)
          ? [styles.item, styles.daySelected]
          : [styles.item, styles.day]
      }
    ...
    >
      <Text
        style={[
          styles.month,
          { color: index + 1 === parseInt(day) ? "white" : "black" },
        ]}
      >
        {item + 1}
      </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        data={[
          ...Array(
            moment(`2023-${monthID + 1}`, "YYYY-M").daysInMonth()
          ).keys(),
        ]}
